...
class SlotMachine {
  ...
  get visibleTile () {
    ...
    const rawContainerMargin = this.$container.css('transform');
  ...
  get visible () {
    const $window = $(window);
    const above = this.$slot.offset().top > $window.scrollTop() + $window.height();
    const below = $window.scrollTop() > this.$slot.height() + this.$slot.offset().top;
